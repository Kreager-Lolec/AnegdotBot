...
def CreateTable():
    ...
    while i < len(listadmin):
        ...
        WHERE
            NOT EXISTS (SELECT * FROM ADMINS WHERE AdminUserName = N'{listadmin[i]}' AND AdminWhoAdd = N'{listwhoadd[i]}' ) ''')
        i = i + 1
    mydb.commit()
...
def checkIfExistsAnedgot(categoryLogical, anegdotLogical):
    ...
    mycursor = mydb.cursor()
    mycursor.execute(
        f'''SELECT COUNT(*) FROM ANEGDOTS WHERE CategoryNameLogical = %s AND AnegdotLogical = %s''', (category, anegdotLogical,))
    if mycursor.fetchone()[0] == 1:
        return True
    return False
